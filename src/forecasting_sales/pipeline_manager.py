...
class NodesManager:
    ...
    def define_init_parameters(
        ...
        if storage == StorageTypes.DATABRICKS:
            ...
            raise ValueError(
                f"The value provided {storage} is not managed. The storage "
    ...
    def execute_pipeline(
        ...
        if storage == StorageTypes.DATABRICKS:
            ...
            raise ValueError(
                f"The value provided {storage} is not valid. "
        ...
        init_params = NodesManager.define_init_parameters(
            storage, spark_session, run_id
        )
        if len(storage_nodes) != len(pipeline.process_nodes):  # type: ignore[arg-type]
            raise ValueError(
                f"The number of precess nodes and storage nodes need to be the same. "
