import numpy as np
from typing_extensions import Any, Self, override, Tuple
import pandas as pd
...
class ForecastingNode(AbstractNode):
    ...
    def process(
        ...
        dict_of_results = {"MAE":[], "MAPE":[], "rmse":[]}
        for fold, (train_index, test_index) in enumerate(tscv.split(X), 1):
            X_train, X_test = X.iloc[train_index], X.iloc[test_index]
            y_train, y_test = y.iloc[train_index], y.iloc[test_index]
            training_data_set = (X_train, X_test, y_train, y_test)
            xgb_model, X_test, y_test = self._train(
                training_data_set,config_params
                        )
        ...
        dict_of_results["MAPE"] = dict_of_results["MAPE"][-1]
        dict_of_results["rmse"] = dict_of_results["rmse"][-1]
        return (dict_of_results,)

    @staticmethod
    def _train(
        training_set: Tuple[pd.DataFrame,pd.DataFrame,
                            pd.DataFrame, pd.DataFrame], training_params: ForecastingConfig
    ) -> xgb.XGBRegressor:
        """
        Method for generating main topics of text
        """
    ...
    @staticmethod
    def _predict(
        xgb_regressor: xgb.XGBRegressor, X_test_data: pd.DataFrame, y_test_data: pd.DataFrame,
        dictionary_of_results: dict[str, list[int]],
    ) -> None:
        """
        Method for generating a table of content
        """
        preds = xgb_regressor.predict(X_test_data)
        mae = mean_absolute_error(y_test_data, preds)
        mask = y_test_data != 0
        mape = np.mean(np.abs((y_test_data[mask] - preds[mask]) / y_test_data[mask])) * 100
        rmse = np.sqrt(mean_squared_error(y_test_data, preds))

        dictionary_of_results["MAE"].append(mae)
        dictionary_of_results["MAPE"].append(mape)
