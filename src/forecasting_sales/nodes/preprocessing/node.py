...
class PreprocessNode(AbstractNode):
    ...
    def process(
        ...
        Process execution
        """
        df_engineered = self._engineer_features(df_to_process)
        df_processed = self._create_temporal_features(df_engineered)
        return (df_processed,)
    ...
    def _engineer_features(
        ...
        df_qte_pivot = (
            df.groupby([pd.Grouper(key="Order Date", freq="M"), "Sub-Category"])[
                "Quantity"
            ]
            .sum()
